...
function BoardPage() {
    ...
    async function fetchLists() {
        try {
            const requestOptions = {
                ...
                body: JSON.stringify({ token: localStorage.getItem("token"), id: localStorage.getItem("board_id") }),
            };

            console.log(localStorage.getItem("token"));
            console.log(localStorage.getItem("board_id"));
            const response = await fetch(
                "http://localhost:3000/getListsByBoardId",
                requestOptions
            );
    ...
    return (
        <div className="board-page">
            ...
            <div className="lists">
                {lists.map((list) => (
                    <List key={list.id} id={list.id} name={list.name} fetchLists={fetchLists}/>
                ))}
